...
<body>
	<jsp:include page="../menu.jsp">
		<jsp:param value="recommend" name="oneMenu"/>
	</jsp:include>
	<!--内容-->
	<div class="cont">
		<div class="select cle">
			<form action="/recommend/index">
				<div class="name_ss">
...
<!-- 					<td width="100">操作</td> -->
				...
				<c:forEach items="${obj.list.list}" var="recommendPurchase" varStatus="status">
					<tr ${status.index%2==1 ? "style='color: #666'" : "style='background-color: #F1F1F1;color: #666'" } >
						<td>${status.index+1}</td>
						<td>${recommendPurchase.userRealName }</td>
						<td>${recommendPurchase.projectName }</td>
						<td>${recommendPurchase.mobile}</td>
						<td>${recommendPurchase.desc}</td>
						<td><fmt:formatDate value="${recommendPurchase.createDate}" pattern="yyyy-MM-dd"/></td>
						<!--
						<td>
			...
			<!-- 分页开始 -->
			<c:url var="urlStr" value="${base}/recommend/index">
